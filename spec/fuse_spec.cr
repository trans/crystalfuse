...
private class RaisingFS < Fuse::FS
  def getattr(path : String) : Fuse::FileAttr | Int32
    raise "boom"
  end
end
...
private class RawFS < Fuse::FS
  ...
  def readdir(path : String, filler : Fuse::DirFiller, fi : Fuse::FileInfo) : Int32
    filler << "." << ".."
    filler << "streamed"
    0
  end
...
private class HandleEchoFS < Fuse::FS
...
private class BytesReadFS < Fuse::FS
  def read(path : String, size : Int32, offset : Int64) : Bytes | Int32
    "abcdef".to_slice[offset.to_i32, Math.min(size, 6 - offset.to_i32)]
  end
end

private class BufferReadFS < Fuse::FS
...
private class LifecycleFS < Fuse::FS
...
describe Fuse::FileAttr do
  it "builds a regular file attr with the right mode and size" do
    attr = Fuse::FileAttr.file(size: 5, mode: 0o444)
  ...
  it "marshals into a struct stat via #to_c" do
    stat = LibC::Stat.new
    attr = Fuse::FileAttr.file(size: 42, mode: 0o444)
  ...
  it "defaults ownership to the mounting process" do
    attr = Fuse::FileAttr.file(size: 0)
...
describe "Bridge.guard" do
  it "turns an uncaught exception into -EIO instead of crashing" do
    Fuse::Bridge.set_instance(RaisingFS.new)
    stat = LibC::Stat.new
    rc = Fuse::Bridge.guard do
      Fuse::Bridge._getattr(
        "/x".to_unsafe, pointerof(stat),
        Pointer(Fuse::Wrap::FileInfo).null
      )
    end
...
describe "Bridge.timespec_to_time" do
  it "decodes a normal timespec" do
    ts = LibC::Timespec.new(tv_sec: 1_000_000_000, tv_nsec: 0)
    Fuse::Bridge.timespec_to_time(ts).should eq(Time.unix(1_000_000_000))
  end

  it "returns nil for UTIME_OMIT (leave unchanged)" do
    ts = LibC::Timespec.new(tv_sec: 0, tv_nsec: Fuse::Bridge::UTIME_OMIT)
    Fuse::Bridge.timespec_to_time(ts).should be_nil
  end
end

describe Fuse::FileInfo do
  it "decodes access-mode flags and gets/sets fh" do
    cfi = Fuse::Wrap::FileInfo.new
    cfi.flags = LibC::O_RDWR | LibC::O_APPEND
    info = Fuse::FileInfo.new(pointerof(cfi))
...
describe "file handles" do
  it "round-trips fh from open to read and exposes the open flags" do
    fs = HandleEchoFS.new
    Fuse::Bridge.set_instance(fs)

    cfi = Fuse::Wrap::FileInfo.new
    cfi.flags = LibC::O_WRONLY
    Fuse::Bridge._open("/f".to_unsafe, pointerof(cfi)).should eq(0)
    fs.seen_writable.should be_true
    cfi.fh.should eq(0xCAFE) # the handle the fs stashed survives in the C struct

    buf = Bytes.new(4)
    Fuse::Bridge._read("/f".to_unsafe, buf.to_unsafe, LibC::SizeT.new(4), 0_i64, pointerof(cfi))
...
describe "read paths" do
  it "fills the kernel buffer via the friendly Bytes-returning form" do
    Fuse::Bridge.set_instance(BytesReadFS.new)
    cfi = Fuse::Wrap::FileInfo.new
    buf = Bytes.new(6)
    n = Fuse::Bridge._read("/f".to_unsafe, buf.to_unsafe, LibC::SizeT.new(6), 0_i64, pointerof(cfi))
    n.should eq(6)
    String.new(buf).should eq("abcdef")
  end

  it "fills the kernel buffer directly via the buffer-filling escape hatch" do
    Fuse::Bridge.set_instance(BufferReadFS.new)
    cfi = Fuse::Wrap::FileInfo.new
    buf = Bytes.new(3)
    n = Fuse::Bridge._read("/f".to_unsafe, buf.to_unsafe, LibC::SizeT.new(3), 2_i64, pointerof(cfi))
    n.should eq(3)
...
describe "raw escape hatches" do
  it "fills the struct stat directly via the raw getattr form" do
    Fuse::Bridge.set_instance(RawFS.new)
    stat = LibC::Stat.new
    rc = Fuse::Bridge._getattr(
      "/f".to_unsafe, pointerof(stat), Pointer(Fuse::Wrap::FileInfo).null)
    rc.should eq(0)
...
describe "xattr bridge (two-call size protocol)" do
  it "stores, size-probes, and reads back a value" do
    Fuse::Bridge.set_instance(XattrFS.new)
    v = "bar".to_slice
    Fuse::Bridge._setxattr("/f".to_unsafe, "user.foo".to_unsafe,
      v.to_unsafe, LibC::SizeT.new(v.size), 0).should eq(0)

    # size 0 → required length, buffer untouched (pass null)
    Fuse::Bridge._getxattr("/f".to_unsafe, "user.foo".to_unsafe,
      Pointer(UInt8).null, LibC::SizeT.new(0)).should eq(3)

    buf = Bytes.new(8)
    n = Fuse::Bridge._getxattr("/f".to_unsafe, "user.foo".to_unsafe,
  ...
  it "returns -ERANGE when the buffer is too small" do
    Fuse::Bridge.set_instance(XattrFS.new)
    v = "abcdef".to_slice
    Fuse::Bridge._setxattr("/f".to_unsafe, "user.k".to_unsafe,
      v.to_unsafe, LibC::SizeT.new(v.size), 0)
    buf = Bytes.new(2)
    Fuse::Bridge._getxattr("/f".to_unsafe, "user.k".to_unsafe,
  ...
  it "serializes the name list NUL-separated" do
    Fuse::Bridge.set_instance(XattrFS.new)
    one = "1".to_slice
    Fuse::Bridge._setxattr("/f".to_unsafe, "user.a".to_unsafe, one.to_unsafe, LibC::SizeT.new(1), 0)
    Fuse::Bridge._setxattr("/f".to_unsafe, "user.bb".to_unsafe, one.to_unsafe, LibC::SizeT.new(1), 0)

    total = Fuse::Bridge._listxattr("/f".to_unsafe, Pointer(UInt8).null, LibC::SizeT.new(0))
    total.should eq(("user.a".bytesize + 1) + ("user.bb".bytesize + 1))

    buf = Bytes.new(total)
    Fuse::Bridge._listxattr("/f".to_unsafe, buf.to_unsafe, LibC::SizeT.new(total)).should eq(total)
...
describe "mknod / link bridge" do
  it "dispatches mknod and link with their arguments" do
    ...
    Fuse::Bridge._mknod("/n".to_unsafe, LibC::ModeT.new(0o100644), LibC::DevT.new(0)).should eq(0)
    fs.last_mknod.should eq({"/n", 0o100644, 0_u64})

    Fuse::Bridge._link("/a".to_unsafe, "/b".to_unsafe).should eq(0)
    fs.linked.should eq({"/a", "/b"})
  end
end

# Exercises the advanced ops, including an Int64 return and a pointer out-param.
private class OpsFS < Fuse::FS
  def lseek(path : String, offset : Int64, whence : Int32, fi : Fuse::FileInfo) : Int64
    offset + 100
  end

  def fallocate(path : String, mode : Int32, offset : Int64, length : Int64, fi : Fuse::FileInfo) : Int32
    mode + 1
  end
...
describe "advanced ops bridge" do
  it "dispatches lseek and returns an Int64 offset" do
    Fuse::Bridge.set_instance(OpsFS.new)
    cfi = Fuse::Wrap::FileInfo.new
    Fuse::Bridge._lseek("/f".to_unsafe, 50_i64, 0, pointerof(cfi)).should eq(150_i64)
  end

  it "dispatches fallocate" do
    Fuse::Bridge.set_instance(OpsFS.new)
    cfi = Fuse::Wrap::FileInfo.new
    Fuse::Bridge._fallocate("/f".to_unsafe, 7, 0_i64, 4096_i64, pointerof(cfi)).should eq(8)
  end

  it "dispatches bmap with an in/out block index" do
    Fuse::Bridge.set_instance(OpsFS.new)
    idx = 41_u64
    Fuse::Bridge._bmap("/f".to_unsafe, LibC::SizeT.new(512), pointerof(idx)).should eq(0)
    idx.should eq(42)
  end

  it "guard64 converts an exception into -EIO" do
    Fuse::Bridge.guard64 { raise "boom" }.should eq(-Errno::EIO.value.to_i64)
  end
end

describe Fuse::Bridge do
  it "dispatches getattr through the bridge and fills the stat buffer" do
    Fuse::Bridge.set_instance(TestFS.new)
