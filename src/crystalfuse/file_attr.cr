module Crystalfuse
  ...
  struct FileAttr
    ...
    def initialize(@mode : Int32, @size : Int64, @nlink : Int32,
                   ...
                   @uid : UInt32 = DEFAULT_UID, @gid : UInt32 = DEFAULT_GID)
    end
    ...
    def self.dir(nlink = 2, mode = 0o755, time = Time.utc,
                 uid = DEFAULT_UID, gid = DEFAULT_GID) : self
      new((LibC::S_IFDIR | mode).to_i32, 0_i64, nlink, time, time, time,
        uid.to_u32, gid.to_u32)
    end
    ...
    def self.file(size : Int, mode = 0o644, time = Time.utc,
                  uid = DEFAULT_UID, gid = DEFAULT_GID) : self
      attr = new((LibC::S_IFREG | mode).to_i32, size.to_i64, 1, time, time, time,
        uid.to_u32, gid.to_u32)
    ...
    def self.symlink(size : Int, mode = 0o777, time = Time.utc,
                     uid = DEFAULT_UID, gid = DEFAULT_GID) : self
